...
let
  ...
  portainer =
    ...
    then bultins.replaceStrings [ "<EDGE-HOSTNAME>"] [currentHost.portainer.edgeUrl ] (builtins.replaceStrings [ "<HOSTNAME>" ] [ currentHost.portainer.dashboardUrl ] (builtins.readFile ./manifests/portainer.yaml))
    else "";
in
{
  imports = [ ./. ];
